...
function levelUp(message, utente){
  ...
  let lvlEmbed = new Discord.RichEmbed()
    ...
    .setFooter(Math.floor(Math.floor(xpNec)-Math.floor(xp[utente.id].xp))+" XP per il prossimo livello", utente.displayAvatarURL);
          ...
          if (!talkedRecently.has(message.author.id)) {
            ...
            if(xp[message.author.id].xp>=nextLvXp){
              ...
              let lvlEmbed = new Discord.RichEmbed()
                ...
                .setFooter(Math.floor(Math.floor(xpNec)-Math.floor(xp[message.author.id].xp))+" XP per il prossimo livello", message.author.displayAvatarURL);
                message.channel.send(lvlEmbed).then(msg => {
                  msg.delete(20000)
                });
            }
